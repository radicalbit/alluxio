...
/**
 * Unit tests for {@link MasterSource} with kerberos environment.
 */
public final class MasterSourceTestKR extends KerberosSecurityTestcase {
  ...
  public void before() throws Exception {
    ...
    try {
      ...
      final String masterPrincipal = "master";

      final File masterKeytab = new File(keyTabDir, masterPrincipal + ".keytab");
      if (masterKeytab.exists() && !masterKeytab.delete()) {
        // log.warn("Unable to delete {}", masterKeytab);
      }

      getKdc().createPrincipal(masterKeytab, masterPrincipal);
      ...
      UserGroupInformation masterUgi = UserGroupInformation.loginUserFromKeytabAndReturnUGI(
              HadoopSecurityTestUtils.qualifyUser(masterPrincipal), masterKeytab.getAbsolutePath());
      ...
      HadoopSecurityTestUtils.runAs(masterUgi,
          new HadoopSecurityTestUtils.AlluxioSecuredRunner<Void>() {
            ...
            public Void run() throws IOException {
              ...
              System.out.println("currentuser just at the beginning inside "
              ...
              MasterContext.getConf().set(Constants.SECURITY_AUTHENTICATION_TYPE,
                  AuthType.KERBEROS.toString());

              System.out.println("auth type"
                  + MasterContext.getConf().get(Constants.SECURITY_AUTHENTICATION_TYPE));

              MasterContext.getConf().set(Constants.SECURITY_AUTHORIZATION_PERMISSION_ENABLED,
                  Boolean.TRUE.toString());

              Journal blockJournal =
                  new ReadWriteJournal(mTestFolder.newFolder().getAbsolutePath());
              ...
              // set up worker
              mWorkerId = mBlockMaster.getWorkerId(new WorkerNetAddress().setHost("localhost")
   ...
   */
  @Test(expected = alluxio.exception.AccessControlException.class)
  public void createFileTest() throws Exception {
    MasterContext.getConf().set(Constants.SECURITY_AUTHENTICATION_TYPE,
        AuthType.KERBEROS.toString());

    System.out
        .println("auth type" + MasterContext.getConf().get(Constants.SECURITY_AUTHENTICATION_TYPE));
  ...
  public void filePersistedTest() throws Exception {
    ...
    mFileSystemMaster.setAttribute(NESTED_FILE_URI,
        SetAttributeOptions.defaults().setPersisted(true));
  ...
  public void freeTest() throws Exception {
    ...
    Command heartBeat2 =
        mBlockMaster.workerHeartbeat(mWorkerId, ImmutableMap.of("MEM", Constants.KB * 1L),
